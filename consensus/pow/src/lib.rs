...
impl State {
    ...
    #[must_use]
    /// PRE_POW_HASH || TIME || 32 zero byte padding || NONCE
    pub fn calculate_spectrex(&self, nonce: u64) -> Uint256 {
    ...
    #[must_use]
    /// PRE_POW_HASH || TIME || 32 zero byte padding || NONCE
    pub fn calculate_astrobwt(&self, nonce: u64) -> Uint256 {
        // Hasher already contains PRE_POW_HASH || TIME || 32 zero byte padding; so only the NONCE is missing
        let hash = self.hasher.clone().finalize_with_nonce(nonce);
        let bwt_hash = astrobwtv3::astrobwtv3_hash(&hash.as_bytes());
    ...
    pub fn calculate_pow(&self, nonce: u64) -> Uint256 {
        match self.header_version {
            constants::BLOCK_VERSION_SPECTREXV1 => self.calculate_spectrex(nonce),
            constants::BLOCK_VERSION_SPECTREXV2 => self.calculate_astrobwt(nonce),
