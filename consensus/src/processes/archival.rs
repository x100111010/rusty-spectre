...
impl ArchivalManager {
    pub fn new(
        ...
        Self { storage, max_block_level, genesis_hash, is_archival, sigma_activation, matrix_activation, thread_pool }
    }
    ...
    fn calc_accepted_id_merkle_root(&self, daa_score: u64, mut accepted_tx_ids: Vec<Hash>, selected_parent: Hash) -> Hash {
        if self.sigma_activation.is_active(daa_score) {
            spectre_merkle::merkle_hash(
                ...
                spectre_merkle::calc_merkle_root(accepted_tx_ids.iter().copied()),
            )
        } else {
    ...
    fn add_archival_block(&self, ArchivalBlock { block, child: _child, acceptance_data }: &ArchivalBlock) -> ArchivalResult<()> {
        ...
        let block_level = calc_block_level(&block.header, self.max_block_level, &self.matrix_activation);
