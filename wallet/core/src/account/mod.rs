...
pub trait Account: AnySync + Send + Sync + 'static {
    ...
    async fn commit_reveal(
        ...
        commit_reveal_batch_bundle(
            pssb::CommitRevealBatchKind::Parameterized { address, commit_amount_sompi },
            reveal_fee_sompi,
            script_sig,
            payload,
            fee_rate,
        ...
        while let Some(result) = stream.next().await {
            match result {
                Ok(psst) => {
                    ...
                    let transaction =
                        psst_to_pending_transaction(psst, self.wallet().network_id()?, change, self.utxo_context().clone().into())?;
                    log_info!("Submitting to rpc");
                    ids.push(transaction.try_submit(&self.wallet().rpc_api()).await?);
                    log_info!("Submitted to rpc");
                }
                Err(e) => {
                    log_info!("Error processing a PSST from bundle: {:?}", e);
                }
            }
        }
        Ok(ids)
