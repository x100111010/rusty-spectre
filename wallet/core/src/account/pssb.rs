...
use spectre_wallet_psst::bundle::{script_sig_to_address, unlock_utxo_outputs_as_batch_transaction_pssb};
use spectre_wallet_psst::prelude::lock_script_sig_templating_bytes;
...
pub fn psst_to_pending_transaction(
    ...
    let (utxo_entries_ref, aggregate_input_value): (Vec<UtxoEntryReference>, u64) = inner_psst
        .inputs
        .iter()
        .filter_map(|input| {
    ...
    let output: &Vec<spectre_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
...
pub async fn commit_reveal_batch_bundle(
    ...
    let bundle_unlock = unlock_utxo_outputs_as_batch_transaction_pssb(
