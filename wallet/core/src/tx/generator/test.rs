...
use spectre_consensus_core::mass::UtxoCell;
...
fn test_generator_compound_200k_10spr_transactions() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Spectre(5.0)),
        [(output_address, Spectre(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
#[test]
fn test_generator_fee_rate_compound_200k_10spr_transactions() -> Result<()> {
    generator(
        test_network_id(),
        &[10.0; 200_000],
        &[],
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    generator(test_network_id(), &inputs, &[], None, Fees::sender(Spectre(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(Spectre(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    ...
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Spectre(0.0)), [(output_address, Spectre(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Spectre(5.0)), [(output_address, Spectre(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Spectre(5.0)),
        [(output_address, Spectre(9_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .drain(
...
fn test_generator_fan_out_1() -> Result<()> {
    use spectre_consensus_core::mass::calc_storage_mass;
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(Spectre(1.0)), [
    //     (output_address, Spectre(0.20000000)),
    //     (output_address, Spectre(0.25000000)),
    //     (output_address, Spectre(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: Spectre(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(Spectre(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();
