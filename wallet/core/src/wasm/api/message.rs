...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/spectre-keypair-standard accounts are currently supported"));
        }
    };
...
try_from!(args: AccountsPssbSignResponse, IAccountsPssbSignResponse, {
    ...
    let response = IAccountsPssbSignResponse::default();
    response.set("pssb", &args.pssb.into())?;
    Ok(response)
});
